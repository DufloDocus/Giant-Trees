...
public class TreeGrow {
    ...
    public TreeGrow(BukkitScheduler b){
        CT = new CreateTree(b, false, GiantTrees.getSettings().getWaitAfterGrow() * 1000);
    }
    /**
     * Checks to see if a block or sappling placed is near another block/sappling
     * and should be a Giant Tree
     * Known Bug: Tree grows at location of last block placed, not the sappling
     * @param block the block
     * @param isSapling is it the sappling or not
    ...
    public boolean nearGrow(Block block, boolean isSapling, Player player){
        if(isSapling){
            if(isBlockNear(block, GiantTrees.getSettings().getBlockNearGrowId(), -1)){
            ...
            if(isBlockNear(block, 6, 1)){
                //known bug, will create tree where block is placed, not sappling
                createTree(block.getLocation(), player, block);
                return true;
            }
            else{
    ...
    private void createTree(Location location, Player player, Block block){
        Tree tree;
        player.sendMessage(ChatColor.GREEN + GiantTrees.getSettings().getGrowMessage());
        if(block.getTypeId() == 6){
            ...
            tree = new Tree(location,
                            getRandom(GiantTrees.getSettings().getMaximumGrowHeight(),
                            GiantTrees.getSettings().getMinimumGrowHeight()),
                            getRandom(GiantTrees.getSettings().getMaximumGrowWidth(),
                            GiantTrees.getSettings().getMinimumGrowWidth()),
                            type, type, "20");
        }
        else{
            tree = new Tree(location,
