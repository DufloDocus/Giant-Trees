...
public class CreateTree implements Runnable {
    ...
    public CreateTree(BukkitScheduler b) {
        ...
        BS = b;
        treeBuildDelay = GiantTrees.getSettings().getTreeBuildDelay();
    }
    ...
    public void addTree(Tree t) {
        ...
        if (!running) {
            try {
                ...
                GiantTrees.logSevere("Unable to start tree creation thread!");
                e.printStackTrace();
            }
        } else {
    ...
    /**
     * Creates a tree using other classes. As an independent thread it can wait
     * while the blocks in question are put into a list and then spawned in the
     * world. This thread must not crash, or it will shutdown the server.
     */
    public void run() {
        running = true;
        Player player;
        ...
        do {
            ...
            taskNumber = BS.scheduleSyncRepeatingTask(GiantTrees.getPlugin(), TB, 0, treeBuildDelay);
    ...
    private void wait(int time) {
        try {
            ...
            GiantTrees.logWarning("Unable to wait!");
            e.printStackTrace();
        }
    }

    /**
     * There is currenly trees in the cache or a tree being built
     *
    ...
    /**
     * Sends a message to a player if there is a player. Avoids error if the
     * tree being created doesn't have a specified owner
     *
