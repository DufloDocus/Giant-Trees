...
public class Undo implements Runnable {
    ...
    public void run() {
        if (FH.pathExists("Saves" + FH.separator + player.getWorld().getName() + FH.separator + "Trees.dat")) {
            ...
            if (closest != -1) {
                ...
                if (owner.equalsIgnoreCase(player.getName()) || GiantTrees.checkPermission(player, "undoall")) {
    ...
    private void doUndo(World world, String[] data) {
        ...
        BS.scheduleSyncDelayedTask(GiantTrees.getPlugin(), UBC);
        do {
            wait(50);
        } while (!UBC.isDone());
    }
    ...
    private double getNum(String s) {
        try {
            ...
            GiantTrees.logSevere("Corrupt File Entry: " + s);
            return 0;
        }
    }
    ...
    private void wait(int time) {
        try {
            ...
            GiantTrees.logSevere("Could not wait!");
            e.printStackTrace();
        }
    }
